...
model_apply <- function(cluster, num_col_importance, num_lag, num_times) {
  ...
  # Columns to use, depending on the 'num_col_importance'
  col_importance <- data_col_importance[[cluster_name(cluster)]][1:200]
  # Subset selection
  data_train <- cluster_mean(cluster, data_solar_add_train) %>% 
    .$data %>% 
    add_lag(num_lag, num_times, col='Cluster') %>%
  ...
  # Model training
  model <- randomForest(Cluster ~ ., data = data_train_val)
  # Results
...
saveRDS(model_applied, '~/Downloads/model_applied_rf_1.rds')
...
saveRDS(out, '~/Downloads/out_rf_1.rds')
# out <- readRDS(file.path('storage', 'out_25_xgb.rds'))

write.table(x = out, file = '~/Downloads/rf_out_1.csv', sep = ',', dec = '.', row.names = FALSE, quote = FALSE)
# 
