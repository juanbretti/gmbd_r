...
# Define grid seach
cost_values <- 10^seq(from = -2, to = 2, length.out = 12)
epsilon_values <- 10^seq(from = -2, to = 0, length.out = 12)
gamma_values <- 10^seq(from = -3, to = 1, length.out = 12)
...
model_best <- lapply(1:num_clusters, function(x) {model_trained[[cluster_name(1)]] %>% 
    .$trained %>%
...
# str(model_trained)
# saveRDS(model_trained, file.path('storage', 'model_trained_23_svm.rds'))
model_trained <- readRDS(file.path('storage', 'model_trained_23_svm.rds'))
...
# saveRDS(model_applied, file.path('storage', 'model_applied_23_svm.rds'))
model_applied <- readRDS(file.path('storage', 'model_applied_23_svm.rds'))
...
# saveRDS(out, file.path('storage', 'out_23_xgb.rds'))
out <- readRDS(file.path('storage', 'out_23_xgb.rds'))
