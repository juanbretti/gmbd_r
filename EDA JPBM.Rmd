---
title: "Programming R Workgroup Project: EDA"
author: "Group E"
date: "3/20/2020"
...
There is only one row that could be considered outliers. I don't see the need of removing outliers.
...
# saveRDS(data_solar_importance, file.path('storage', 'data_solar_importance.rds'))
data_solar_importance <- readRDS(file.path('storage', 'data_solar_importance.rds'))
...
m_ <- 5
# data_mice <- mice(data, m=m_, maxit=10, meth='pmm', seed=500)
# saveRDS(data_mice, file.path('storage', 'data_add_mice.rds'))
data_mice <- readRDS(file.path('storage', 'data_add_mice.rds'))
# summary(data_mice)

# Average of all the Multivariate Imputation
data_mice_ <- 0
for (i in 1:m_) data_mice_ <- data_mice_ + complete(data_mice, i)
data_mice_ <- data_mice_/m_
```

## Outliers
If I remove outliers before doing 'mice', I will remove all the row or will have columns with different lenght/nrows.
Doing outliers removal, after 'mice', means that 'mice' was trained with the outliers.

@Li, this does not work, because I have different number of NA per column.
So the data.table has a strage dimmension.
This "outliers" thing should be done after "mice".
