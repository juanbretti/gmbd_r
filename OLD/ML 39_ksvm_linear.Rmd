...
var_read <- function(x, r = !case_save) {
  # readRDS(file = file.path('storage', 'additional_variables.RData'))
...
# Cleanup
if(clean_variables) rm(list = c('data', 'data_mice_', 'm_', 'maxit_', 'i', 'data_mice', 'data_add', 'num_mice_iterations'))
```
...
# Cleanup
if(clean_variables) rm(list = c('data', 'p_elbow', 'p_silhouette', 'p_gap_statistic', 'pre_', 'data_solar_add_train_'))
```
...
if(clean_variables) rm(list=c('nrow_train', 'nrow_val', 'nrow_test', 'data_solar_add_train_', 'data_add_col_dates'))
```
...
# var_read('data_col_importance', TRUE)
if(clean_variables) rm(list=c('cl', 'select_important', 'time_importance', 'data'))
```

# Hyperparameter optimization
Using svm library, for all the 'Clusters'
...
# Define grid seach
C_values <- 2^seq(from = -5, to = 10, length.out = 4)
# C_ <- C_values[1]
# cluster=1
# cost <- 10^-3
# epsilon <- 10^(1)
...
model_train <- function(cluster) {
  ...
  # Call the training
  trained <- foreach(C_ = C_values, .combine = rbind, .packages=(.packages()), .export=ls(envir=globalenv())) %dopar% {
        
        # Model training
        model <- kernlab::ksvm(Cluster ~ ., data = data_train, C = C_, kernel = kernlab::vanilladot())
        # Results
...
model_apply <- function(cluster, num_col_importance, num_lag, num_times) {
  ...
  C_ <- model_best[[cluster_name(cluster)]]$C

  # Model training
  model <- kernlab::ksvm(Cluster ~ ., data = data_train_val, C = C_, kernel = kernlab::vanilladot())
  # Results
...
var_save('model_applied')
if(clean_variables) rm(list=c('cl', 'model_apply', 'model_result'))
```
