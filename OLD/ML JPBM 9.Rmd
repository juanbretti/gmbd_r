...
time_length(interval(range(data_solar$Date2)), unit = "year")
2012-1994
table(data_solar$Month)
str(data_solar$Month)
```
...
# saveRDS(model_trained, file.path('storage', 'model_trained2.rds'))
model_trained <- readRDS(file.path('storage', 'model_trained2.rds'))

rm(list=c('cl', 'model_train'))
```

## Results

```{r}
model_result <- function(WeatherStation, col_importance_number) {
  ...
  model_train <- model_trained[[WeatherStation]]$model
  ...
  result_combined <- tibble(mse_train, mse_val, mae_train, mae_val)
  # Quick plot for 'Test'
  result_plot <- ggplot() +
...
result_combined <- data_solar_col_produ %>% 
  map_dfr(function(x) bind_cols(WeatherStation = x, model_result(x, 40)$combined)) %>% 
  mutate(mae_ratio = mae_val/mae_train,
         fitting = ifelse(mae_ratio<1, 'Underfitting', ifelse(mae_ratio==0, 'Fit', 'Overfitting'))) %>% 
  arrange(mae_ratio)

ggplot(result_combined) + 
...
model_result <- function(model_train, model_train_val, data_train, data_val, data_train_val, data_test) {
  ...
  # Combining the results
  result_combined <- tibble(mse_train, mse_val, mse_train_val, mse_test, mae_train, mae_val, mae_train_val, mae_test)
  # Quick plot for 'Test'
  result_plot <- ggplot() +
...
model_apply <- function(WeatherStation, col_importance_number = 40) {
  ...
  # Results
  model_result <- model_result(trained_$model, model_train_val, data_train, data_val, data_train_val, data_test)
  
  return(list(
    WeatherStation = WeatherStation,
    train_time = train_time,
    model = model_train_val,
    result = model_result
  ))
}
...
saveRDS(model_applied, file.path('storage', 'model_applied2.rds'))
# model_applied <- readRDS(file.path('storage', 'model_applied2.rds'))
...
result_combined <- data_solar_col_produ %>% 
  ...
  mutate(mae_ratio = mae_test/mae_train_val,
         fitting = ifelse(mae_ratio<1, 'Underfitting', ifelse(mae_ratio==0, 'Fit', 'Overfitting'))) %>% 
  arrange(mae_ratio)
