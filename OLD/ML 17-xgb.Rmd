...
model_train <- function(WeatherStation, col_importance_number = 40) {
  ...
  # xgbLinear
  # tuneControl <- expand.grid(lambda = 10^seq(from = -5, to = 0, length.out = 10),
  #                   alpha = 10^seq(from = -5, to = 0, length.out = 10),
  #                   nrounds = 50,
  #                   eta = seq(from = 0, to = 0.3, length.out = 10))
  
  # xgbDART
  len <- 5
  tuneControl <- expand.grid(nrounds = floor((1:len) * 50),
                   max_depth = seq(1, len),
...
saveRDS(model_trained, file.path('storage', 'model_trained18_xgbDART.rds'))
# model_trained <- readRDS(file.path('storage', 'model_trained17_xgb.rds'))
