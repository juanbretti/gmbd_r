...
# Define grid seach
mtry_values <- 1:(num_col_importance-1)
...
model_train <- function(cluster, num_col_importance, num_lag, num_times) {
  ...
  # Call the training
  trained <- foreach(mtry_ = mtry_values, .combine = rbind, .packages=(.packages()), .export=ls(envir=globalenv())) %dopar% {
        
        # Model training
        model <- randomForest(Cluster ~ ., data = data_train, mtry = mtry_)
        # Results
...
# str(model_trained)
saveRDS(model_trained, file.path('storage', 'model_trained_28_rf.rds'))
# model_trained <- readRDS(file.path('storage', 'model_trained_28_rf.rds'))
...
saveRDS(model_applied, file.path('storage', 'model_applied_28_rf.rds'))
# model_applied <- readRDS(file.path('storage', 'model_applied_28_rf.rds'))
...
saveRDS(out, file.path('storage', 'out_28_rf.rds'))
# out <- readRDS(file.path('storage', 'out_28_rf.rds'))
