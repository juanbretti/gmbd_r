...
# Define grid seach
len <- 10
alpha_values <- seq(0, 1, length = len)
lambda_values <- c(0, 10 ^ seq(-1, -4, length = len - 1))
...
model_train <- function(cluster, num_col_importance, num_lag, num_times) {
  ...
  # Call the training
  trained <- foreach(alpha_ = alpha_values, .combine = rbind) %:%
    foreach(lambda_ = lambda_values, .combine = rbind, .packages=(.packages()), .export=ls(envir=globalenv())) %dopar% {
        
      h2o.init()
      alpha_ <- lambda_ <- 0
        # Model training
        frame_name <- paste0("tmp_glmnet_dat_",sample.int(100000, 1))
        tmp_train_dat = h2o::as.h2o(data_train, destination_frame = frame_name)
        model <- h2o::h2o.glm(x = setdiff(colnames(data_train), 'Cluster'), y = 'Cluster',
                              training_frame = tmp_train_dat, alpha = alpha_, lambda = lambda_)
        # Results
...
# str(model_trained)
saveRDS(model_trained, file.path('storage', 'model_trained_29_h2o.glm.rds'))
# model_trained <- readRDS(file.path('storage', 'model_trained_29_h2o.glm.rds'))
...
model_apply <- function(cluster, num_col_importance, num_lag, num_times) {
  ...
  mtry <- model_best[[cluster_name(cluster)]]$mtry

  # Model training
  model <- randomForest(Cluster ~ ., data = data_train_val, mtry = mtry)
  # Results
...
saveRDS(model_applied, file.path('storage', 'model_applied_29_h2o.glm.rds'))
# model_applied <- readRDS(file.path('storage', 'model_applied_29_h2o.glm.rds'))
...
saveRDS(out, file.path('storage', 'out_29_h2o.glm.rds'))
# out <- readRDS(file.path('storage', 'out_29_h2o.glm.rds'))
