...
model_train <- function(WeatherStation, col_importance_number = 40) {
  ...
  # # Grid search
  # tuneControl <- expand.grid(C = 10^seq(from = -2, to = 1, by = 0.1/2),
  #                           sigma = 10^seq(from = -3, to = -1, by = 0.1/2))
...
system.time({
  model_trained <- lapply(data_solar_col_produ[1:10], model_train)
})
names(model_trained) <- data_solar_col_produ[1:10]

stopCluster(cl)

saveRDS(model_trained, file.path('storage', 'model_trained4.rds'))
...
model_apply <- function(WeatherStation, col_importance_number = 40) {
  ...
  model_coef <- data.table(
    ncomp = trained_$model$finalModel$bestIter$ncomp
  )
...
saveRDS(model_applied, file.path('storage', 'model_applied4.rds'))
