...
# saveRDS(model_trained, file.path('storage', 'model_trained_21_SVM.rds'))
model_trained <- readRDS(file.path('storage', 'model_trained_21_SVM.rds'))
...
# saveRDS(model_applied, file.path('storage', 'model_applied_21_SVM.rds'))
model_applied <- readRDS(file.path('storage', 'model_applied_21_SVM.rds'))
...
system.time({
  foreach(ws_=data_solar_col_produ[1:3], .combine = cbind) %dopar% {
    for (row_ in 5114:nrow(data_solar_add)) {
      data[row_, ws_] <- predict_fun(row_, ws_)$predict_fun
    }
    data[, ws_]
    print(ws_)
  }
...
# saveRDS(out, file.path('storage', 'out_21_SVM.rds'))
# out <- readRDS(file.path('storage', 'out_21_SVM.rds'))
