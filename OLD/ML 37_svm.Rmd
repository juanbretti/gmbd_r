...
saveRDS(data_col_importance, file.path('storage', 'data_col_importance_lag_37_svm.rds'))
# data_col_importance <- readRDS(file.path('storage', 'data_col_importance_lag_37_svm.rds'))
...
# Define grid seach
cost_values <- 10^seq(from = -2, to = 1, length.out = 3)
epsilon_values <- 10^seq(from = -2, to = 0, length.out = 3)
gamma_values <- 10^seq(from = -3, to = -1, length.out = 3)
...
model_train <- function(cluster) {
  ...
  trained <- foreach(cost = cost_values, .combine = rbind) %:%
      foreach(epsilon = epsilon_values, .combine = rbind, .packages=(.packages()), .export=ls(envir=globalenv())) %dopar% {
        
        # Model training
        model <- svm(Cluster ~ ., data = data_train, cost = cost, epsilon = epsilon, kernel = 'linear')
        # Results
...
# str(model_trained)
saveRDS(model_trained, file.path('storage', 'model_trained_37_svm.rds'))
# model_trained <- readRDS(file.path('storage', 'model_trained_37_svm.rds'))
...
model_apply <- function(cluster, num_col_importance, num_lag, num_times) {
  ...
  # Model training
  model <- svm(Cluster ~ ., data = data_train_val, cost = cost, epsilon = epsilon, gamma = gamma, kernel = 'linear')
  # Results
...
saveRDS(model_applied, file.path('storage', 'model_applied_37_svm.rds'))
# model_applied <- readRDS(file.path('storage', 'model_applied_37_svm.rds'))
...
saveRDS(out, file.path('storage', 'out_37_svm.rds'))
# out <- readRDS(file.path('storage', 'out_37_svm.rds'))
