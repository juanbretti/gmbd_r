...
# Define grid seach
cost_values <- 10^seq(from = -3, to = 3, length.out = 5)
epsilon_values <- 10^c(-1,0)
# cluster=1
# cost <- 10^-3
# epsilon <- 10^(1)
...
model_apply <- function(cluster, num_col_importance, num_lag, num_times) {
  ...
  # Model training
  model <- svm(Cluster ~ ., data = data_train_val, cost = cost, epsilon = epsilon, kernel = 'linear')
  # Results
