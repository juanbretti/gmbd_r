...
col_importance <- data_col_importance[[WeatherStation]][1:col_importance_number]
  ...
  predictions_train <- predict(model_trained[[WeatherStation]]$model, newdata = data_train)
  predictions_val <- predict(model_trained[[WeatherStation]]$model, newdata = data_val)
  # Get errors
...
model_apply <- function(WeatherStation, col_importance_number = 40) {
  ...
  # Model
  train_time <- system.time({
  model_train_val <- caret::train(WeatherStation ~ .,
                      data = data_train_val,
                      method = "svmRadialSigma",
                      metric = "RMSE",
                      tuneLength = 30,
...
# saveRDS(model_applied, file.path('storage', 'model_applied.rds'))
model_applied <- readRDS(file.path('storage', 'model_applied.rds'))
