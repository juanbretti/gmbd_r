...
model_train <- function(WeatherStation) {
  # Subset selection
  data_train <- bind_cols(WeatherStation = data_solar_add_train[[WeatherStation]],
                    select(data_solar_add_train, all_of(c(data_solar_col_predi, data_add_col))))
  data_val <- bind_cols(WeatherStation = data_solar_add_val[[WeatherStation]],
                    select(data_solar_add_val, all_of(c(data_solar_col_predi, data_add_col))))

  # PCA
  a <- select(data_solar_add_train, all_of(c(data_solar_col_predi, data_add_col)))
  skim(a)
  pre_model_train <- preProcess(x = a, method = c('pca'))
  pre_model_train <- preProcess(a, method = c('pca'))
  pre_model_trained <- predict(pre_model_train, a)
  colnames(a)
  # pre_model_train <- prcomp(a, scale = TRUE)
  pre_model_trained <- as.data.frame(predict(pre_model_train, newdata = a))
  pre_model_valed <- predict(pre_model_train, newdata = data_val)
  
  
  a <- preProcess(mtcars, method = "pca")
  b <- predict(a, mtcars)
  ...
  # Results
  model_result <- model_result(model, pre_model_trained, pre_model_valed)
...
system.time({
  model_trained <- lapply(data_solar_col_produ[1:2], model_train)
})
names(model_trained) <- data_solar_col_produ[1:2]

stopCluster(cl)

saveRDS(model_trained, file.path('storage', 'model_trained16.rds'))
# model_trained <- readRDS(file.path('storage', 'model_trained16.rds'))
...
model_apply <- function(WeatherStation) {
  ...
  # Subset selection
  data_train <- bind_cols(WeatherStation = data_solar_add_train[[WeatherStation]], data_solar_add_train)
  data_val <- bind_cols(WeatherStation = data_solar_add_val[[WeatherStation]], data_solar_add_val)
  data_train_val <- bind_rows(data_train, data_val)
  data_test <- bind_cols(WeatherStation = data_solar_add_test[[WeatherStation]], select(data_solar_add_test, 'Date', 'Date2'))
...
saveRDS(model_applied, file.path('storage', 'model_applied16.rds'))
# model_applied <- readRDS(file.path('storage', 'model_applied16.rds'))
