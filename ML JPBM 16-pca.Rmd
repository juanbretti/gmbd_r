...
pre_add <- data_add_mice %>% 
  preProcess(method = c('pca'), thresh = 0.9) %>% 
  predict(newdata = data_add_mice) %>% 
  rename_at(vars(starts_with("PC")), function(x) paste0(x,'_add'))

pre_solar <- data_solar %>% 
  rename_at(vars(starts_with("PC")), function(x) paste0(x,'_solar'))

# Column names with sufix
data_add_col_pca <- setdiff(colnames(pre_add), data_add_col_dates)
data_solar_col_predi_pca <- paste0(data_solar_col_predi, '_solar')

# Joining the two data.frames
data_solar_add <- pre_solar %>% 
  left_join(pre_add, by = 'Date', suffix = c("_solar", "_add"))

# Cleanup
rm(list = c('data_solar', 'data_add_mice', 'pre_add', 'pre_solar'))
```
...
data_solar_add_predict <- data_solar_add[5114:nrow(data_solar_add), c(data_solar_col_dates, data_solar_col_predi_pca, data_add_col_pca)]
...
#                                     select_important(dat=select(data_solar_add_train, !!c(data_solar_col_predi_pca, data_add_col_pca)),
...
# saveRDS(data_col_importance, file.path('storage', 'data_col_importance2.rds'))
data_col_importance <- readRDS(file.path('storage', 'data_col_importance2.rds'))
